...
class GetUsersFromTeamsTest(TestCase):
    ...
    def test(self):
        ...
        assert list(Team.objects.get_users_from_teams(org, [team])) == [user2]
        ...
        assert set(list(Team.objects.get_users_from_teams(org, [team]))) == set([user2, user3])
        assert list(Team.objects.get_users_from_teams(org2, [team])) == []
        assert list(Team.objects.get_users_from_teams(org2, [team2])) == []
        self.create_member(user=user, organization=org2, role='member', teams=[team2])
        assert list(Team.objects.get_users_from_teams(org2, [team2])) == [user]
